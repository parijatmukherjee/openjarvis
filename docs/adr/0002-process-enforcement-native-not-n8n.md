...
"Playbook" / process state machine — the same way **GroundingEngine** enforces grounding.
Do **not** use n8n (or any external workflow engine) as the agent's control plane.

**Why native, not n8n:**

- **It is the core thesis.** OpenJarvis exists to make the runtime enforce what the
...
- **Determinism / replay / audit.** Process state must live in the event-sourced
  session (**JarvisStateStore**) and the hash-chained audit (**Audit**) so runs replay
...
- **Reuses what exists.** The phase machine layers directly on the agent loop, the
  GroundingEngine accept-or-reprompt pattern, JarvisStateStore state, the capability gate (**the Lab**),
  and Audit audit — small, on-thesis surface area.

n8n _could_ technically branch on validate-pass/fail and loop, but making it the
control plane sacrifices exactly the properties OpenJarvis is built to guarantee.
...
- Each phase has an **acceptance gate** (the model proposes "done"; the runtime
  decides), mirroring GroundingEngine's grounding modes.
