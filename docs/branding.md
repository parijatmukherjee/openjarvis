...
Canonical brand reference for **OpenJarvis**. It inherits the `openclaw-hawkins`
brand (the _OpenJarvis_ / Jarvis, Indiana motif) and **extends it to the new
...
| **The Agents**        | The specialist agents                                            | `*-agent`                |
| **The Hive**          | The persistence layer (the "the runtime" — memory that survives) | state + memory stores    |
| **JarvisStateStore**  | Durable orchestration state                                      | `state`                  |
| **JarvisMemoryStore** | Decay-aware shared memory                                        | `memory`                 |
...
## 2. New vocabulary (OpenJarvis subsystems)

These name the capabilities OpenJarvis adds over the source. Each has a stable
functional id (the protocol) and a OpenJarvis brand alias (the prose).

| Brand alias         | Functional id             | Subsystem                                                          | Why this name                                                                                                                                                                         |
| ------------------- | ------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **GroundingEngine** | `grounding`               | The anti-hallucination Grounding engine                            | GroundingEngine perceives the truth and keeps reality intact — the engine keeps the agent in the **Right Side Up** (tool-verified reality) instead of the fabricated **the runtime**. |
| **the Gate**        | `firewall`                | Untrusted-content firewall (prompt-injection defense)              | The boundary the the runtime leaks through. The firewall controls what crosses from untrusted external input into the agent's reasoning.                                              |
| **the Lab**         | `sandbox`                 | Capability sandbox + RBAC                                          | Jarvis National Laboratory — controlled containment of dangerous capabilities.                                                                                                        |
| **the Vault**       | `vault`                   | Encrypted secrets vault                                            | Hopper's hidden cabin where the secret (GroundingEngine) is kept locked away and safe.                                                                                                |
...
| **Audit**           | `audit`                   | Tamper-evident, hash-chained audit log                             | Audit Bauman, the meticulous investigator who records and verifies everything.                                                                                                        |
...
| **The AV Club**     | `plugin-sdk` + `registry` | Plugin SDK + loader                                                | Jarvis Middle's AV Club, where Mr. Clarke's students build and wire up new gadgets (capabilities).                                                                                    |
...
| _"Connecting to the Web…"_        | Dispatching to a Agent.                                     |
...
- **CLI binary** (`openjarvis`) and subcommands — short, ergonomic.
- **Env vars**, **table names**, **package names** (`@openjarvis/*`) — technical.
