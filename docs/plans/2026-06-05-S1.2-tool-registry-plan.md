...
**Tech Stack:** TypeScript (strict), Zod (`zod@^3`) for schemas, `zod-to-json-schema` for native-schema export, Vitest. Builds on S1.1 (`@openjarvis/core`, `os/platform.ts`).
...
- **Not in scope (later milestones):** the agent loop that emits `ToolCall`s and feeds `ToolResult`s back to the model (S1.4); GroundingEngine grounding enforcement that _requires_ a `disk_free` call (S1.5/6); sandboxed process isolation (S6 hardening — S1 enforces the capability contract in-process only).

---

## Next plans (after this lands)

- **S1.3** — model-adapter interface + Ollama (local+cloud) + OpenAI-compatible adapter + `FileVault` (the Vault) for keys.
- **S1.4** — the agent loop: native tool-calling round-trip (model emits `ToolCall` → registry → `ToolResult` → model).
- **S1.5 / S1.6** — GroundingEngine grounding (modes, enforcement, citations) + Audit audit + Gate taint.
