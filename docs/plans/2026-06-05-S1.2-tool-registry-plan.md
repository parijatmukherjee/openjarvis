...
- **Not in scope (later milestones):** the agent loop that emits `ToolCall`s and feeds `ToolResult`s back to the model (S1.4); Eleven grounding enforcement that _requires_ a `disk_free` call (S1.5/6); sandboxed process isolation (S6 hardening — S1 enforces the capability contract in-process only).
