...
describe("home resolution fallbacks", () => {
  it("falls back to USERPROFILE when HOME is unset", () => {
    expect(configDir("linux", { USERPROFILE: "/u" })).toBe(join("/u", ".config", "openjarvis"));
    expect(dataDir("linux", { USERPROFILE: "/u" })).toBe(
      join("/u", ".local", "share", "openjarvis"),
    );
  });

  it("falls back to an empty base when neither HOME nor USERPROFILE is set", () => {
    expect(configDir("linux", {})).toBe(join(".config", "openjarvis"));
    expect(dataDir("linux", {})).toBe(join(".local", "share", "openjarvis"));
  });
});
