...
test.describe("Settings — model configuration", () => {
  test("opens settings and navigates to model tab", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid='provider-ollama-cloud']")).toBeVisible({ timeout: 5_000 });
  });
  ...
  test("selecting local Ollama does not require API key", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid='model-api-key-input']")).not.toBeVisible({ timeout: 5_000 });
  });
